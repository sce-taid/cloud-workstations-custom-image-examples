...
@test "Chrome wrapper script contains suppression flags" {
  grep -q "--no-first-run" "${ASSETS_DIR}/usr/bin/google-chrome-stable"
  grep -q "--no-default-browser-check" "${ASSETS_DIR}/usr/bin/google-chrome-stable"
}
