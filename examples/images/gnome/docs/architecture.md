...
1.  **Build-Time Hooks**: Executed _before_ package installation. Used for repository setup, custom binary injection, or pre-requisite configuration.
2.  **Post-Install Hooks**: Executed _after_ all packages are installed but _before_ the desktop integration logic runs. This is the primary point for patching files provided by third-party `.deb` packages.
...
- **Downstream Apps (Children)**: Higher-level layers (like Android Studio for Platform) integrate with this layer via two mechanisms:
