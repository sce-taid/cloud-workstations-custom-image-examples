...
readonly SCRIPTS_PATHS="/google/scripts /build-hooks.d /post-install-hooks.d /etc/workstation-startup.d"
...
main() {
  ...
  # Run post-install hooks for layers that need to patch installed files
