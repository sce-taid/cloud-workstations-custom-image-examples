...
license: Apache-2.0
allowed-tools: examples/preflight/skills/update-preflight/scripts/hotpatch_frontend.sh examples/preflight/skills/update-preflight/scripts/test_and_hotpatch.sh
metadata:
  author: sce-taid <sce@taid.me>
  resources:
    - examples/preflight/docs/design.md
    - examples/preflight/docs/requirements.md
---
...
2.  **What it does**: This script automatically runs the frontend test suite via Vitest (`npm test`). If successful, it builds the frontend assets via Vite (`npm run build`) and syncs the `examples/preflight-web/dist/` directory to `/var/www/html/` on your active workstation.
