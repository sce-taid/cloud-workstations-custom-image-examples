...
import { applyTranslations, renderLanguageList, loadLocaleHashes, fetchTranslations, t, setLanguage, filterLanguages, SUPPORTED_LANGS } from "../i18n_module";
...
describe("i18n Module", () => {
  beforeEach(() => {
    ...
    state.translations['en'] = { name: 'English', native: 'English', dict: MOCK_TRANSLATIONS };
  ...
  test("renderLanguageList handles null container and button click", () => {
    ...
    setupMockDOM();
    renderLanguageList();
    const btn = document.querySelector('#language-list button') as HTMLButtonElement;
    ...
    test("applyTranslations branches", () => {
      ...
      `;
          state.translations["en"] = { name: "en", native: "en", dict: { "key": "val", "status_ready": "R", "other": "O" } };
          state.config = { ...state.config, lang: "en" };
    ...
    test("filterLanguages", () => {
          document.body.innerHTML = '<input id="language-search" value="eng"/><div id="language-list"></div>';
          filterLanguages();
        });
    ...
    test("Requirement: Selecting a language returns to Settings", async () => {
        ...
        window.closeModal = jest.fn((id: string) => closeModal(id));
