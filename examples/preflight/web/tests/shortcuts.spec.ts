...
import { initGlobalShortcuts, renderShortcutsList, filterShortcuts, shortcuts } from "../shortcuts_module";
...
describe("Shortcuts Module", () => {
  beforeEach(() => {
    ...
    window.toggleDebug = jest.fn();
    window.reportBug = jest.fn();
    window.toggleRetry = jest.fn();
  ...
  test("Shortcuts are ignored in inputs/textareas, but Escape blurs", () => {
    ...
    test("initGlobalShortcuts coverage", () => {
          initGlobalShortcuts();
          document.body.innerHTML = '<input id="inp" /><textarea id="txt"></textarea>';
    ...
    test("filterShortcuts", () => {
          document.body.innerHTML = '<input id="shortcut-search" value="s"/><div id="shortcuts-list-container"></div>';
          filterShortcuts();

          document.body.innerHTML = '<input id="shortcut-search" value="x"/><div id="shortcuts-list-container"></div>';
          filterShortcuts();
        });
});
