...
import { openModal, closeModal, renderConnectionTypeList, updateDisplayData, initEventListeners, resetUIVisuals, closeAllModals, syncRetryIntervalState } from "../ui_module";
...
describe("UI Module", () => {
  ...
  test("Desktop icon click triggers manual connect", () => {
    ...
    const manualConnectMock = jest.fn();
  ...
  test("updateDisplayData populates text elements", () => {
    state.config = { ...state.config, hostname: "test-host", uplink: "test-uplink" };
    updateDisplayData();
    expect(document.getElementById('display-hostname')?.textContent).toBe('test-host');
    expect(document.getElementById('display-uplink')?.textContent).toBe('RDP');
  });

  test("syncAdvancedSections toggles exclusively", () => {
    initEventListeners();
    const basic = document.getElementById('basic-settings-details') as HTMLDetailsElement;
    const advanced = document.getElementById('advanced-settings-details') as HTMLDetailsElement;
  ...
  test("renderConnectionTypeList handles missing container and button click", () => {
    ...
    setupMockDOM();
    renderConnectionTypeList();
    const btn = document.getElementById('connection-type-list')?.querySelector('button') as HTMLButtonElement;
    btn.click();
  ...
  test("Global keyboard listener handles space key in settings modal", () => {
    ...
    test("openPrivacyModal via click", async () => {
          document.body.innerHTML = '<div id="privacy-text-content"></div><div id="privacy-modal"></div><button id="btn-open-privacy-from-help"></button>';
          initEventListeners();

          window.fetch = jest.fn().mockResolvedValue({ ok: true, text: async () => "privacy" });
    ...
    test("openModal specific modals", () => {
          document.body.innerHTML = '<div id="license-modal"></div><div id="license-content"></div><div id="license-list-container"></div><div id="component-details"></div><div id="license-viewer"></div><div id="language-modal"></div><input id="language-search"/><div id="shortcuts-modal"></div><input id="shortcut-search"/>';
          openModal("license-modal");
          openModal("language-modal");
          openModal("shortcuts-modal");
        });
    test("syncAdvancedSections branches", () => {
          document.body.innerHTML = '<details id="basic-settings-details"></details><details id="advanced-settings-details"></details>';

          const basic = document.getElementById('basic-settings-details') as HTMLDetailsElement;
          const adv = document.getElementById('advanced-settings-details') as HTMLDetailsElement;
          ...
          document.getElementById('basic-settings-details')?.dispatchEvent(new Event('toggle'));
        });
    ...
    test("closeAllModals", () => {
          document.body.innerHTML = '<div id="m1"></div><div id="settings-modal"></div>';
    ...
    test("updateDisplayData branches", () => {
          document.body.innerHTML = '<div id="display-hostname"></div><div id="display-uplink"></div>';
          updateDisplayData();
        });
    test("resetUIVisuals branches", () => {
          document.body.innerHTML = '<div id="status-icon" class="animate-ready"></div><main><svg></svg></main><div class="starfield-container animate-drift"></div><div id="main-status"></div><div id="live-timer"></div>';
    ...
    test("renderConnectionTypeList coverage", () => {
          ...
          renderConnectionTypeList();
          const btn = document.querySelector("#connection-type-list button") as HTMLButtonElement;
          btn?.click();
        });
    ...
    test("ui_module syncRetryIntervalState missing elements", () => {
        ...
        document.body.innerHTML = '<input id="retry-enable" type="checkbox" /><input id="retry-interval" />';
    ...
    test("Requirement: Retry Interval is disabled when Auto-Redirect is unchecked", () => {
        initEventListeners();
        const redirectCb = document.getElementById('retry-enable') as HTMLInputElement;
        const intervalInput = document.getElementById('retry-interval') as HTMLInputElement;
