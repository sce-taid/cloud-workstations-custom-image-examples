...
import { loadSBOM, viewFullLicenseText, renderComponentDetails, renderLicenseList, backToLicenseList, backToComponentDetails } from "../sbom_module";
...
describe("SBOM Module", () => {
  ...
  test("loadSBOM populates state and renders", async () => {
    const mockManifest: SBOMManifest = {
      ...
      components: [{ name: 'Test', version: '1.0', group: 'G', licenseId: 'MIT', supplier: 'S', url: '', repository: '' }]
    };
    (global.fetch as jest.MockedFunction<typeof fetch>).mockResolvedValue(createMockResponse(mockManifest));
  ...
  test("viewFullLicenseText updates viewer", async () => {
    state.sbom = {
      ...
      components: [{ name: 'MIT', version: '1.0', supplier: 'S', licenseId: 'MIT', repository: '', url: '', group: '', licenseFile: 'mit.txt' }]
    } as SBOMManifest;

    (global.fetch as jest.MockedFunction<typeof fetch>).mockResolvedValue(createMockResponse("FULL TEXT"));
  ...
  test("viewFullLicenseText handles fetch error and non-ok response", async () => {
    state.sbom = {
      ...
      components: [{ name: 'C', version: '1', supplier: 'S', licenseId: 'MIT', url: '', repository: '', group: '' }]
    } as SBOMManifest;
  ...
  test("viewFullLicenseText early returns", async () => {
    ...
    state.sbom = {
      ...state.sbom,
      licenses: { ...state.sbom.licenses, 'MIT': { ...state.sbom.licenses['MIT'], localText: 'mit.txt' } }
    };
    ...
    test("loadSBOM ok", async () => {
          window.fetch = jest.fn().mockResolvedValue({ ok: true, json: async () => ({ metadata: {}, licenses: {}, components: [] }) });
          await loadSBOM();
        });
    test("renderLicenseList missing div", () => {
          state.sbom = { metadata: {}, licenses: {}, components: [{ name: "a", version: "1", group: "", licenseId: "b", supplier: "s", url: "u", repository: "r" }] };
    ...
    test("renderComponentDetails unsplash", () => {
          document.body.innerHTML = '<div id="license-list-container"></div><div id="component-details"></div>';
          state.sbom = {
            metadata: {},
            licenses: { "Unsplash": { name: "Unsplash", url: "", localText: "" } },
            components: [{ name: "test", version: "1", group: "g", licenseId: "Unsplash", supplier: "s", url: "u", repository: "r", licenseFile: "l" }]
          };
          renderComponentDetails("test");
        });
    test("renderComponentDetails missing detailsContainer", () => {
        document.body.innerHTML = '<div id="license-list-container"></div>';
        state.sbom = { metadata: {}, licenses: {}, components: [{ id: 'a', name: 'Lib', version: '1', supplier: 'Dev', licenseId: 'MIT', url: 'http', repository: 'http' } as any] };
        renderComponentDetails('Lib'); // should return early
      });
    test("viewFullLicenseText missing viewerContainer or textContainer", async () => {
        state.sbom = {
          metadata: {},
          components: [{ id: 'c', name: 'Comp', version: '1.0', supplier: 'S', licenseId: 'MIT', url: '', repository: '' } as any],
          licenses: { 'MIT': { name: 'MIT License', localText: 'path/to/mit.txt', url: '' } }
        };
        document.body.innerHTML = '<div id="license-list-container"></div><div id="component-details"></div>';
        await viewFullLicenseText('Comp');
        document.body.innerHTML = '<div id="license-list-container"></div><div id="component-details"></div><div id="license-viewer"></div>';
        await viewFullLicenseText('Comp');
      });
    test("viewFullLicenseText fetch resolves but not ok", async () => {
        ...
        `;
        window.fetch = jest.fn(() => Promise.resolve({ ok: false, text: () => Promise.resolve("") })) as any;
        await viewFullLicenseText('Comp');
      });
    test("viewFullLicenseText handles missing component or licenseInfo", async () => {
        ...
        state.sbom = { ...state.sbom, components: [{ id: 'c', name: 'Comp', version: '1.0', supplier: 'S', licenseId: 'MIT', url: '', repository: '' } as any] };
        await viewFullLicenseText('Comp'); // Missing license
      });
    test("viewFullLicenseText ok", async () => {
          document.body.innerHTML = '<div id="license-list-container"></div><div id="component-details"></div><div id="license-viewer"></div><div id="license-text-content"></div>';
          state.sbom = {
            metadata: {},
            licenses: { "l": { name: "l", url: "l", localText: "/l.txt" } },
            components: [{ name: "test", version: "1", group: "g", licenseId: "l", supplier: "s", url: "u", repository: "r" }]
          };
          window.fetch = jest.fn().mockResolvedValue({ ok: true, text: async () => "text" });
    ...
    test("sbom_module renderComponentDetails Unsplash case", () => {
        state.sbom = {
          metadata: {},
          components: [
            { id: 'comp1', name: 'Photo', version: '1.0', supplier: 'Jane Doe', licenseId: 'Unsplash', url: 'http', repository: 'http' } as any,
            { id: 'comp2', name: 'Other', version: '1.0', supplier: 'John Doe', licenseId: 'MIT', url: 'http', repository: 'http' } as any
          ],
          licenses: {
            'Unsplash': { name: 'Unsplash License', localText: 'path/to/unsplash.txt', url: '' },
    ...
    test("sbom_module renderComponentDetails handles missing elements", () => {
        state.sbom = {
          metadata: {},
          components: [
            { id: 'comp1', name: 'Lib', version: '1', supplier: 'Dev', licenseId: 'MIT', url: 'http', repository: 'http' } as any
          ],
          licenses: {}
        };
        document.body.innerHTML = '';
    ...
    test("sbom_module viewFullLicenseText fetch error handling", async () => {
        ...
        state.sbom = {
          metadata: {},
          components: [
            { id: 'c', name: 'Comp', version: '1.0', supplier: 'S', licenseId: 'MIT', url: '', repository: '' } as any
          ],
          licenses: {
            'MIT': { name: 'MIT License', localText: 'path/to/mit.txt', url: '' }
          }
        ...
        await viewFullLicenseText('Comp');
        expect(document.getElementById('license-text-content')?.textContent).toContain('Network error');
      });
});
