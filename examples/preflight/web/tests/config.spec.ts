...
import {
  ...
  updateConfig,
  copySettingsUrl,
  reportBug, getCurrentUIValues, updateStatusMessage, toggleRedirect, clearAllData, mapStepToTimeout, mapTimeoutToStep, mapStepToRetry, mapRetryToStep, applyUIValues } from "../config_module";
...
describe("Config Module", () => {
  ...
  beforeEach(() => {
    setupMockDOM();
    localStorage.clear();
    state.config = { ...DEFAULT_CONFIG, connectionTypes: ['RDP', 'SSH', 'VNC'] };
    state.simulateSec = 0;
    ...
    searchSpy = jest.spyOn(windowUtils, 'search', 'get').mockReturnValue("");
    jest.spyOn(windowUtils, 'origin', 'get').mockReturnValue("http://127.0.0.1");
    jest.spyOn(windowUtils, 'pathname', 'get').mockReturnValue("/");
  ...
  test("loadConfig parses all semantic URL parameters correctly", () => {
    searchSpy.mockReturnValue("?autoRedirect=false&timeout=150&retryInterval=500&simulateDelay=12&protocol=SSH&lang=es");
    loadConfig();
  ...
  test("updateConfig reloads with new URL parameters and persists language", () => {
    ...
    const checkbox = document.getElementById('retry-enable') as HTMLInputElement;
    checkbox.checked = false;

    updateConfig();

    const replaceStateMock = window.history.replaceState as jest.Mock;
  ...
  test("loadConfig sets language from CWS_CONFIG.serverLang", () => {
    ...
    localStorage.setItem('cws_preflight_config', JSON.stringify({ lang: 'es' }));
    loadConfig();
    expect(state.config.lang).toBe('es');
  });
  ...
  test("loadConfig catches parse error from localStorage", () => {
    ...
    loadConfig();
    expect(consoleSpy).toHaveBeenCalledWith('Failed to parse config', expect.any(Error));
    ...
    test("loadConfig invalid timeout & lang", () => {
          ...
          window.history.pushState({}, "", "/?timeout=abc&retryInterval=abc&simulateDelay=abc");
          loadConfig();
          expect(state.config.timeoutMs).toBe(DEFAULT_CONFIG.timeoutMs);
        });
    test("getCurrentUIValues branches", () => {
          document.body.innerHTML = '<div id="connection-type-list"><button class="border-secondary">RDP</button></div>';
    ...
    test("copySettingsUrl with tooltip", () => {
          jest.useFakeTimers();
          Object.defineProperty(navigator, 'clipboard', { value: { writeText: jest.fn().mockResolvedValue(true) }, configurable: true });
    ...
    test("updateStatusMessage fallback window.t", () => {
        ...
        jest.spyOn(msg, 'getAttribute').mockReturnValue(null);
        updateStatusMessage();
    ...
    test("Requirement: Copy URL uses UI values, Bug Report uses saved Config", () => {
        ...
        copySettingsUrl();
        const copiedUrl = new URL((navigator.clipboard.writeText as jest.Mock).mock.calls[0][0] as string);
        expect(copiedUrl.searchParams.get('autoRedirect')).toBe('false');

        window.open = jest.fn() as unknown as typeof window.open;
        reportBug();
        const bugUrl = new URL((window.open as jest.Mock).mock.calls[0][0] as string);
