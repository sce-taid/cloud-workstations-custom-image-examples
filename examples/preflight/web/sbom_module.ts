...
interface RawSBOM {
  ...
  readonly licenses: Record<string, {
...
export function renderLicenseList(): void {
  ...
  if (!state.sbom) {
    container.innerHTML = '<p class="text-secondary animate-pulse p-8 uppercase tracking-widest text-center">Loading software manifest...</p>';
    return;
  }
  ...
  groupNames.forEach(groupName => {
    ...
    if (groupComponents) {
      groupComponents.forEach((comp: ComponentInfo) => {
        const license = state.sbom?.licenses[comp.licenseId]?.name || comp.licenseId;
        const item = document.createElement('button');
        item.className = 'w-full text-left p-4 rounded-lg bg-white/5 border border-neutral-800 hover:border-secondary transition-all group';
...
export function renderComponentDetails(name: string): void {
    ...
    <div class="space-y-8">
      ...
      <div class="grid grid-cols-2 gap-6">
        <div>
          ...
          <p class="text-sm text-white font-mono uppercase tracking-wider">${comp.version}</p>
        </div>
        <div>
      ...
      <div>
        ...
        <p class="text-sm text-on-surface-variant uppercase tracking-widest">${comp.supplier}</p>
      </div>

      <div>
        <h3 class="ui-label mb-3">Project Links</h3>
        <div class="space-y-3">
          <a href="${comp.url}" target="_blank" class="flex items-center justify-between p-3 rounded bg-white/5 border border-neutral-800 hover:border-secondary transition-all group">
          ...
          <a href="${comp.repository}" target="_blank" class="flex items-center justify-between p-3 rounded bg-white/5 border border-neutral-800 hover:border-secondary transition-all group">
  ...
  document.getElementById('btn-back-to-license-list')?.addEventListener('click', () => backToLicenseList());
  document.getElementById('btn-view-full-license')?.addEventListener('click', () => { void viewFullLicenseText(comp.name); });
}
 ...
 */
export async function viewFullLicenseText(componentName: string): Promise<void> {
  const component = state.sbom?.components.find(c => c.name === componentName);
  if (!component) return;
  ...
  if (!listContainer || !detailsContainer || !viewerContainer || !textContainer) return;

  listContainer.classList.add('hidden');
  detailsContainer.classList.add('hidden');
  viewerContainer.classList.remove('hidden');

  textContainer.textContent = (window.t?.('msg_loading_license', 'Loading license for...') || 'Loading...') + ` ${componentName}...`;
  ...
  try {
    ...
    if (response.ok) {
      ...
      textContainer.textContent = (window.t?.('msg_error_load_license', 'Error loading license') || 'Error');
    }
  } catch (e) {
    textContainer.textContent = (window.t?.('msg_error_network_license', 'Network error') || 'Network error');
  }
}
