...
async function openPrivacyModal(): Promise<void> {
  ...
  if (contentEl) {
    contentEl.innerHTML = '<p class="text-secondary animate-pulse p-8 uppercase tracking-widest text-center">Loading Privacy Notice...</p>';
    try {
      const response = await fetch('/privacy_notice.html');
      if (response.ok) {
        contentEl.innerHTML = await response.text();
      } else {
        contentEl.innerHTML = '<p class="text-red-400 p-8 text-center uppercase tracking-widest">Error loading privacy notice.</p>';
      }
    } catch (e) {
      contentEl.innerHTML = '<p class="text-red-400 p-8 text-center uppercase tracking-widest">Network error.</p>';
    }
  }
...
export function syncRetryIntervalState(): void {
  ...
  if (redirectCb instanceof HTMLInputElement && intervalInput instanceof HTMLInputElement && intervalGroup) {
 ...
 */
function syncAdvancedSections(expandedId: 'basic-settings-details' | 'advanced-settings-details'): void {
  if (typeof document === 'undefined') return;
  ...
  if (expandedId === 'basic-settings-details' && basic instanceof HTMLDetailsElement && basic.open && advanced instanceof HTMLDetailsElement) {
    advanced.open = false;
  } else if (expandedId === 'advanced-settings-details' && advanced instanceof HTMLDetailsElement && advanced.open && basic instanceof HTMLDetailsElement) {
    basic.open = false;
  }
}
...
export function renderConnectionTypeList(): void {
  ...
  state.config.connectionTypes.forEach(proto => {
    ...
    btn.className = `w-full text-left p-3 rounded mb-2 border transition-colors ${state.config.connectionId === proto ? 'bg-secondary/20 border-secondary text-secondary' : 'bg-white/5 border-neutral-800 text-white hover:bg-white/10'}`;
...
export function initEventListeners(): void {
  ...
  document.getElementById('btn-close-language')?.addEventListener('click', () => closeModal('language-modal'));
  document.getElementById('btn-close-license')?.addEventListener('click', () => closeModal('license-modal'));
  document.getElementById('btn-close-licenses-top')?.addEventListener('click', () => closeModal('license-modal'));
  document.getElementById("back-to-details-btn")?.addEventListener("click", () => backToComponentDetails());
  ...
  document.getElementById('btn-close-privacy')?.addEventListener('click', () => closeModal('privacy-modal'));

  // Modal Open Buttons
  document.getElementById('btn-open-settings')?.addEventListener('click', () => { openModal('settings-modal'); syncRetryIntervalState(); });
  document.getElementById('btn-open-settings-timer')?.addEventListener('click', () => { openModal('settings-modal'); syncRetryIntervalState(); });
  document.getElementById('btn-open-settings-uplink')?.addEventListener('click', () => { openModal('settings-modal'); syncRetryIntervalState(); });
  document.getElementById('btn-open-language')?.addEventListener('click', () => openModal('language-modal'));
  document.getElementById('btn-settings-open-language')?.addEventListener('click', () => openModal('language-modal'));
  document.getElementById('btn-open-license')?.addEventListener('click', () => openModal('license-modal'));
  document.getElementById('btn-open-shortcuts')?.addEventListener('click', () => openModal('shortcuts-modal'));
  document.getElementById('btn-open-help')?.addEventListener('click', () => openModal('help-modal'));

  // Bridge Listeners (Help Modal)
  document.getElementById('btn-open-licenses-from-help')?.addEventListener('click', () => openModal('license-modal'));
  document.getElementById('btn-open-shortcuts-from-help')?.addEventListener('click', () => openModal('shortcuts-modal'));
  document.getElementById('btn-open-privacy-from-help')?.addEventListener('click', () => { void openPrivacyModal(); });

  // Settings Actions
  document.getElementById('btn-update-config')?.addEventListener('click', () => { updateConfig(); closeAllModals(); });
  document.getElementById('btn-reset-config')?.addEventListener('click', () => clearAllData());
  document.getElementById('btn-copy-settings-url')?.addEventListener('click', () => {
    copySettingsUrl();
  });
  document.getElementById('btn-report-bug')?.addEventListener('click', () => window.reportBug?.());
  document.getElementById('btn-retry-toggle')?.addEventListener('click', () => toggleRetry());

  // Main Dashboard Interaction
  document.getElementById('desktop-icon-trigger')?.addEventListener('click', () => window.manualConnect?.());
  document.getElementById('btn-manual-connect')?.addEventListener('click', () => window.manualConnect?.());
  document.getElementById('btn-manual-connect-msg')?.addEventListener('click', () => window.manualConnect?.());
  ...
  // Dynamic UI Sync
  document.getElementById('retry-enable')?.addEventListener('change', () => syncRetryIntervalState());
  document.getElementById('basic-settings-details')?.addEventListener('toggle', () => syncAdvancedSections('basic-settings-details'));
  document.getElementById('advanced-settings-details')?.addEventListener('toggle', () => syncAdvancedSections('advanced-settings-details'));
