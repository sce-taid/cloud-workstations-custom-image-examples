...
async function run() {
  ...
  const markedPath = path.resolve(process.cwd(), 'node_modules/marked/lib/marked.esm.js');
  ...
  const renderer = {
    ...
    strong(token) {
      return `<strong class="text-white font-bold">${this.parser.parseInline(token.tokens)}</strong>`;
    }
  };
  ...
  try {
    ...
    console.log(`Successfully generated professional HTML fragment: ${outputPath}`);
  } catch (err) {
