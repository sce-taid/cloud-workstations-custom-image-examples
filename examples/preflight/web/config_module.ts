...
export function loadConfig(): void {
  ...
  if (window.CWS_CONFIG) {
    config = {
      ...
      connectionTypes: window.CWS_CONFIG.supportedProtocols ?? config.connectionTypes,
  ...
  if (redirectParam !== null) {
    config = { ...config, autoRedirect: !(redirectParam === '0' || redirectParam === 'false') };
  }
  ...
  const simulateParam = params.get('simulateDelay') ?? params.get('simulateSec');
  if (simulateParam) {
  ...
  if (debugParam !== null) {
    config = { ...config, showDebug: (debugParam === 'true' || debugParam === '1') };
  }
...
export function getCurrentUIValues(): UIValues {
  const redirectEl = document.getElementById('retry-enable') as HTMLInputElement | null;
  const debugEl = document.getElementById('debug-enable') as HTMLInputElement | null;
  const timeoutEl = document.getElementById('timeout-limit') as HTMLInputElement | null;
  const intervalEl = document.getElementById('retry-interval') as HTMLInputElement | null;
  const simulateEl = document.getElementById('simulate-sec') as HTMLInputElement | null;
  ...
  if (connectionList) {
    ...
    if (activeBtn?.textContent) {
      ...
      const found = state.config.connectionTypes.find(t =>
        window.t?.(`label_protocol_${t.toLowerCase()}`, t.toUpperCase()) === activeBtn.textContent ||
  ...
  return {
    ...
    timeoutMs: timeoutEl ? mapStepToTimeout(parseInt(timeoutEl.value, 10)) * 1000 : state.config.timeoutMs,
    retryIntervalMs: intervalEl ? mapStepToRetry(parseInt(intervalEl.value, 10)) : state.config.retryIntervalMs,
    simulateSec: simulateEl ? parseInt(simulateEl.value, 10) : state.simulateSec,
...
export function applyUIValues(data: unknown): void {
  ...
  const redirectEnableEl = document.getElementById('retry-enable') as HTMLInputElement | null;
  const debugEnableEl = document.getElementById('debug-enable') as HTMLInputElement | null;
  const timeoutLimitEl = document.getElementById('timeout-limit') as HTMLInputElement | null;
  const timeoutValueEl = document.getElementById('timeout-value');
  const simulateSecEl = document.getElementById('simulate-sec') as HTMLInputElement | null;
  const simulateValueEl = document.getElementById('simulate-value');
  const retryIntervalEl = document.getElementById('retry-interval') as HTMLInputElement | null;
...
function getSerializedUrl(source: 'config' | 'ui'): string {
  ...
  const data = source === 'ui' ? getCurrentUIValues() : {
  ...
  url.searchParams.set('retryInterval', Math.round(data.retryIntervalMs).toString());
...
export function updateStatusMessage(): void {
  ...
  if (state.isHealthy) {
    ...
    msgEl.setAttribute('data-i18n', state.config.autoRedirect ? 'status_message' : 'status_message_manual');
  }
...
export function reportBug(): void {
  ...
  const body = `URL: ${url}`;
  window.open(`https://github.com/sce-taid/cloud-workstations-custom-image-examples/issues/new?body=${encodeURIComponent(body)}`, '_blank');
}
