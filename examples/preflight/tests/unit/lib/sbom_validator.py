        ...
        with sbom_path.open("r", encoding="utf-8") as f:
            ...
            for lid, info in licenses.items():
                ...
                if not asset_path.is_file():
                    print(f"error: missing asset for {lid}: {asset_path}", file=sys.stderr)
                    sys.exit(1)

                if asset_path.stat().st_size == 0:
                    print(f"error: empty asset for {lid}: {asset_path}", file=sys.stderr)
                    sys.exit(1)
                ...
                with asset_path.open("r", encoding="utf-8") as tf:
                    ...
                    if "404" in content and "not found" in content:
                        print(f"error: asset for {lid} is a 404 page: {asset_path}", file=sys.stderr)
                        sys.exit(1)

                    # Signature Check
                    sig = signatures.get(lid, "").lower()
                    if sig and sig not in content:
                        print(f"error: asset for {lid} missing signature '{sig}': {asset_path}", file=sys.stderr)
                        sys.exit(1)

            # Integrity Check: Ensure components reference valid IDs
            for comp in data.get("components", []):
                if comp.get("license_id") not in licenses:
                    print(f"error: component {comp['name']} unknown license: {comp['license_id']}", file=sys.stderr)
                    sys.exit(1)
