...
**GitSeep** is a sophisticated **Source Code History Orchestrator**. It uses a geological analogy to help developers maintain a pristine, structurally sound source code history without the risks and manual toil of complex interactive rebasing. By mathematically projecting changes into their correct historical strata, GitSeep enables a high-velocity workflow where you can focus entirely on your logic, while the tool ensures your history remains atomic, self-contained, and perfectly organized for review.
...
**The Result**: Your history becomes an unconsolidated **Alluvial** timeline where architectural changes are scattered across multiple commits.

Without automation, retroactively sorting these changes is a painful process. However, given an automated approach, developers—and particularly AI agents in an **agentic development workflow**—can embrace a **Flow-State Workflow**: simply stay on HEAD, focus entirely on the evolution of the source code, and let GitSeep handle the complex "stratigraphy" of the repository. GitSeep autonomously sorts your changes back into their logical historical chapters before you publish your work, ensuring a flawless narrative for your reviewers.
...
- **Conflict-Prone**: Rebase works by replaying patches. If intermediate commits changed the same files, you must resolve merge conflicts at _every_ step where the code diverged.
...
In a perfect repository, every commit is an **atomic, self-contained source code chapter**. If a commit says "Google Cloud Workstations - GNOME Blueprint," it should contain _all_ files and scripts related to that specific layer, regardless of when they were actually written during the development process.
...
- **Bedrock** (`🪨`): A specific historical Stratum that "owns" a source code path (e.g., `examples/images/gnome/`). Logic from other strata will eventually settle here.
...
- **Seepage** (`🫧`): The upward movement or surfacing of logic. A change made deep in the history (e.g., a file introduced too early) "seeps up" through the strata to settle into a _newer_ Bedrock commit.
- **Lithification** (`💎`): The process by which scattered historical versions of a file are pressurized and cemented into a single, permanent record within a Bedrock stratum. This is equivalent to a "squash" or "compress" operation, ensuring that the final Bedrock state represents the absolute truth of the source code layer. This is a core purpose of GitSeep and is performed automatically.
- **Sedimentation** (`🏞️`): The process of syncing or "settling" commits from a main developer branch into isolated feature branches. Using a Tree-Based (DAG) Stacked PR model, GitSeep autonomously updates feature branch pointers—and correctly cherry-picks dependencies—to exactly mirror the refined Bedrock commits in your unified linear history. This preserves all chronological dependencies between commits automatically without error-prone interactive rebasing.
- **Alluvium** (`🏖️`): Unconsolidated, scattered changes that haven't yet been organized into a Bedrock stratum. This represents the "noisy" state of a repository before GitSeep is applied.
- **Outcrop** (`⛰️`): A visible, stable reference point in the history (e.g., `origin/main`) that serves as the foundation for new stratigraphy.
- **Erosion** (`🌪️`): The process of removing redundant or transient historical layers (e.g., "fixup" commits) during the reconstruction phase.
- **Subsidence** (`📉`): The natural settling of feature branches as they are merged into the upstream Outcrop, allowing the Rolling Base to move forward.
- **Tectonics** (`🌋`): Large-scale structural changes to the repository's Directed Acyclic Graph (DAG), such as re-parenting multiple feature branches or shifting the entire stack to a new Outcrop.
- **Metamorphism** (`🧪`): The structural transformation of the repository history during the reconstruction phase, where original commits are pressurized and reorganized into a refined, logical stratigraphy.
...
## 6. The Solution: GitSeep Tool

GitSeep automates **Synthetic History Reconstruction**. It rebuilds your branch stratum-by-stratum, ensuring that:

1.  **Topology Validation**: It maps your declared Stacked PR dependencies and mathematically predicts if your chosen structure will cause merge conflicts _before_ touching Git history.
2.  **Bedrock Strata** are updated to include the _final_ state of their assigned paths.
3.  **Intermediate Strata** are "filtered" to either pass files down (Percolate) or elevate them up (Seep) to their correct Bedrock.
4.  **Historical Integrity** is preserved (same commit count, same messages, same authorship).
...
- **Traceability**: Self-contained commits make it clear _why_ a file was changed in the context of a specific feature, rather than appearing as a mysterious "fix" later in the timeline.

By using GitSeep, you respect your reviewer's time and increase the likelihood of a fast, high-quality approval.

## 7. Benefits

- **Zero-Mutation & Zero-Loss Guarantee**: GitSeep mathematically compares trees and parents before writing to the database. If a Stacked PR or linear commit has not structurally changed, the engine safely reuses the original commit hash. Furthermore, GitSeep guarantees **Zero-Loss**: any new files created at your surface (HEAD) that are not managed by your rules are automatically detected and injected into the final orchestrated commit.
- **Concurrent Performance & Reliability**: GitSeep utilizes an optimized, thread-safe concurrent discovery engine. It can scan large histories in parallel while maintaining strict memory integrity, a claim verified by high-volume concurrency stress tests.
- **Safe Worktree Updates**: GitSeep performs a safe system-level checkout to synchronize your workspace without touching untracked files. If a checkout failure occurs, it will explicitly list the problematic dirty files to help you stage or stash them.
- **No Rebase Conflicts**: Since it uses "State Projection" (checkout) rather than "Patch Replay" (rebase) for linear reconstruction, you never have to manually resolve merge conflicts.
- **Autonomous Stacked PRs**: Define dependencies using the `parent` key, and GitSeep will automatically weave a complex Directed Acyclic Graph (DAG) of feature branches for you via safe, verified cherry-picks.
...
- **Single-Key Interaction**: Prompts are responsive and only require a single keypress (no `Enter` required). In Phase 2, you can navigate with `Page Up/Down`, quickly toggle items with `Space` or `x`, view diffs with `v` or `d`, finalize with `c`, or automatically accept everything with the `a` (Accept All) shortcut.
...
- **Safety First**: It mathematically guarantees topology success in Phase 0 and requires interactive confirmation for every stratum in Phase 1 and 2.
...
Create a `.gitseep.yaml` file in your repository root. Use `.gitseep.yaml.example` as a template. You can use the standard list format or the advanced Sedimentation dictionary format to bind a Bedrock commit to a feature branch and define its topological parent:
...
"2026-04-14 17:36:44 +0000":
  ...
  parent: feat/android-studio-for-platform # Explicit Topological Dependency (Stacked PR)
  paths:
...
The ideal scenario is that you perform all your commits and seeping _before_ you publish your work. In this case, you can safely push your clean, refactored history with a standard command:
...
### Step 8: Post-Merge Synchronization (Stacked PRs)

When your bottom-most Stacked PRs are approved and merged into the upstream repository (e.g., `origin/main`), your local developer branch needs to be synchronized without accidentally rewriting the now-permanent history.

GitSeep automates this via the **Rolling Base** concept. You do not need to delete or comment out old rules; `.gitseep.yaml` acts as a permanent, append-only log of your architectural decisions!

1.  **Configure the Base**: Ensure your `.gitseep.yaml` includes the `config` block pointing to your upstream target:
    ```yaml
    config:
      base_ref: origin/main
    ```
2.  **Pull & Rebase**: Fetch the latest changes and rebase your local branch:
    ```bash
    git fetch origin
    git rebase origin/main dev
    ```
    _(Git's native rebase will intelligently drop the local commits that were merged upstream, leaving only your unmerged strata)._
3.  **Run GitSeep**: Run the tool normally. GitSeep will dynamically anchor to the new `origin/main`, mathematically prove which rules in your `.gitseep.yaml` belong to commits that are already merged, and **silently ignore them**. It will only percolate changes for your remaining, active feature branches!

### Step 9: Pre-Commit Integration (`gitseep check`)

To enforce that developers do not commit code that violates the `.gitseep.yaml` stratigraphy rules, you can integrate GitSeep into your pre-commit pipeline.

The `check` command mathematically proves that your Stacked PR topology is valid (no cycles, no cherry-pick conflicts) and ensures that your working tree is clean and your branch history is fully sedimented—all in milliseconds, without mutating your repository.

If you are using the [pre-commit](https://pre-commit.com/) framework, you can consume the exported hook by adding this to your repository's `.pre-commit-config.yaml`:

<!-- prettier-ignore -->
```yaml
- repo: https://github.com/sce-taid/cloud-workstations-custom-image-examples
  rev: <target_revision_or_tag>
  hooks:
    - id: gitseep       # Manual full reconstruction (only runs when requested)
    - id: gitseep-check # Runs on every commit to validate stratigraphy
```

**Manual Execution**:
To manually trigger a full history reconstruction via the pre-commit hook (which automatically applies `--amend` and `--auto-approve`):

```bash
pre-commit run --hook-stage manual gitseep
```
...
| Option                  | Description                                                                                       |
| :---------------------- | :------------------------------------------------------------------------------------------------ |
| `--branch <name>`, `-B` | Target branch name to update (defaults to current branch).                                        |
...
| `--quiet`, `-q`         | Suppress all non-error output. This option automatically enables `--auto-approve`.                |
...
| `--dry-run`, `-n`       | Simulate the refactor without modifying history.                                                  |
| `--verbose`, `-v`       | Enable debug logging.                                                                             |
| `--amend`, `-a`         | Amend uncommitted changes into the last commit before running.                                    |
| `--experimental-go-git` | Use buggy go-git for status/checkout instead of system git (requires explicit confirmation).      |
...
- **File-Level Granularity vs. Line-Level Patching**: GitSeep maps rules at the file or directory level. It does not "split" file logic. It lithifies (squashes) all historical versions of a file into the defined Bedrock. If a developer modifies `file A` heavily in Stratum 1, and then adds a new feature to `file A` in Stratum 5, assigning `file A` to Bedrock Stratum 1 will pull the _entire HEAD state_ of that file into Stratum 1. If a file genuinely contains logic that belongs in two different strata, the developer must manually split the file architecturally before running GitSeep. (Note: You can use the `--no-lithify` option to prevent automatic squashing and enforce a strict 1-to-1 migration policy).
- **Intermediate Build Breakage (The Bisect Problem)**: Because GitSeep moves the "final" state of a file back in time, that file might now reference a function or class in another file that _hasn't been introduced yet_ in the reconstructed timeline. While the final `HEAD` is guaranteed to be 100% correct, intermediate commits might fail to compile or pass tests. This breaks `git bisect` functionality.
...
- **The Publishing Boundary**: Seeping rewrites history. It is highly beneficial _until_ the commits are published or pushed to a shared collaborative branch (where other developers rely on the history, or where branch protection rules prevent `--force`).
  - _Exception:_ In **Gerrit**, developers push to a dedicated review branch. Until a change is reviewed and submitted, it is fully expected that developers will rewrite history and upload new patches.
...
- **[`hg absorb` / `git-absorb`](https://github.com/tummychow/git-absorb)**: Originally popularized by Mercurial (`hg absorb`) and ported to Git, these tools use heuristics (like `git blame`) to automatically figure out which recent commit a set of uncommitted working directory changes belong to, silently creating `fixup!` commits or amending them in place.
  - **The Difference in Mechanism**: `absorb` looks at _uncommitted_ line-level changes and relies on heuristics to guess the target commit. GitSeep looks at _already committed_ file-level history and relies on deterministic, developer-defined architectural rules (via `.gitseep.yaml`).
