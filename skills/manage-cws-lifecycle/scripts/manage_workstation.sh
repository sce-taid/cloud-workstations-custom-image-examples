...
usage() {
  echo "Usage: $0 [start|stop|restart|wait|tunnel] [WORKSTATION] [flags/args]"
  echo ""
  echo "Actions:"
...
main() {
  ...
  case "${action}" in
    ...
    wait)
      ...
      ;;
    tunnel)
      tunnel_ws "${workstation}" "${project}" "${cluster}" "${config}" "${region}" "${timeout}" "${local_port}" "${remote_port}" "${browser}" "${manage_ssh_config}"
      ;;
    *)
      usage
      ;;
